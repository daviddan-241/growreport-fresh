  ...
  const scan = async () => {
    ...
    const html = `
      <div style="font-family:Arial;padding:40px;background:#f9f9f9;max-width:600px;margin:auto">
        ...
        <p style="margin-top:20px"><b>Want daily reports + AI captions?</b><br>
           Get PRO → <a href="https://growreport-fresh.vercel.app/pro">growreport-fresh.vercel.app/pro</a></p>
    ...
    try {
      await axios.post('https://api.resend.com/emails', form, {
        headers: { Authorization: `Bearer ${process.env.NEXT_PUBLIC_RESEND_KEY}` }
      });
      alert('Report sent!');
    } catch (err) {
      alert('Error. Check key.');
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-600 to-blue-600 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full">
        ...
        <input placeholder="facebook.com/YourPage" className="w-full p-4 border rounded-lg mb-4" onChange={e => setUrl(e.target.value)} />
        <input placeholder="you@gmail.com" className="w-full p-4 border rounded-lg mb-6" onChange={e => setEmail(e.target.value)} type="email" />

        <button onClick={scan} disabled={loading || !email.includes('@')} className="w-full bg-black text-white py-4 rounded-lg font-bold hover:bg-gray-800 disabled:opacity-50">
        ...
        <div className="mt-8 p-6 bg-gray-50 rounded-lg text-sm">
          ...
          <div className="mt-3 flex gap-2">
            <input value={`https://growreport-fresh.vercel.app/?ref=${email.split('@')[0] || 'you'}`} readOnly className="flex-1 p-2 bg-white border rounded text-xs" />
            <button onClick={() => navigator.clipboard.writeText(`Free growth tool: https://growreport-fresh.vercel.app/?ref=${email.split('@')[0] || 'you'}`)} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
              Copy
            </button>
          </div>
        </div>
